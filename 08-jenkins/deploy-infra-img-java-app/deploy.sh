...
echo "Executando ansible ::::: [ ansible-playbook -i hosts provisionar.yml -u jenkins --private-key ssh -i ~/.ssh/id_rsa ]" 
ANSIBLE_HOST_KEY_CHECKING=False ansible-playbook -i hosts provisionar.yml -u jenkins --private-key ssh -i ~/.ssh/id_rsa 
