...
echo "Executando ansible ::::: [ ansible-playbook -i hosts provisionar.yml -u ubuntu --private-key ssh -i /var/lib/jenkins/.ssh/id_rsa ]" 
ANSIBLE_HOST_KEY_CHECKING=False ansible-playbook -i hosts provisionar.yml -u ubuntu --private-key ssh -i /var/lib/jenkins/.ssh/id_rsa 
